...
def seg_postprocess(
    ...
    if masks.numel() > 0:  # Check if masks tensor is non-empty
        masks = F.interpolate(masks.unsqueeze(0), image_size, mode='bilinear', align_corners=False).squeeze(0)
        masks = masks.gt_(0.5)[..., None]
    else:
        masks = torch.empty((0, image_size[1], image_size[0], 1), dtype=torch.float32)
