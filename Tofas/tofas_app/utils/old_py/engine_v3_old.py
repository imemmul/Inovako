...
FEATURE_DIR = "/home/inovako/Desktop/TOFAS_Basler_Save.pfs"
# TODO ProcessPoolExecutor
# TODO possible optimization creating inference procecss for each cam
# TODO grabbing result should wait for the trigger result, SoftwareTrigger also flash
# FIXME possible queue overflow
    ...
    while running.is_set() or q.qsize() > 0:
        try:
            ...
            if len(bboxes) == 0:
                ...
                count += 1
                cv2.imwrite(filename=f"{args.out_dir}output_{count}_{capture_id}_{cam_id}_{exp_time}_no_det.jpg", img=image)
            else:
                ...
                for (bbox, score, label) in zip(bboxes, scores, labels):
                    ...
                    cv2.putText(draw,
                                ...
                                thickness=2)
                count += 1
                cv2.imwrite(filename=f"{args.out_dir}output_{count}_{capture_id}_{cam_id}_{exp_time}_det_.jpg", img=draw)
        except Exception as e:
...
def run_devices(cam_array, nums_cams, args):
    ...
    """
    q = Queue()
    p = Process(target=run_inference, args=(q, args, running)) # background listening
    p.start()
    cam_array.StartGrabbing(py.GrabStrategy_LatestImageOnly) # ??
    # Define a function to be run in each thread
    def trigger_and_capture(cam:py.InstantCamera, cam_id, running, delay_dict, capture_test):
        capture_amount = 0
        try:
            while running.is_set():
                ...
                if capture_test.is_set():
                    ...
                    #cam.ExecuteSoftwareTrigger()
                    grabResult = cam.RetrieveResult(500, py.TimeoutHandling_ThrowException)
            ...
            print(f"some bugs in trigger_and_capture: cam_id: {cam_id} {e}")
        ...
        try:
            for cam_id, cam in enumerate(cam_array):
                ...
                if cam_id != args.master:
                    ...
                    executor.submit(trigger_master, args, cam, cam_id, running, delay_dict, q, capture_test)
        except Exception as e:
...
def trigger_master(args, cam, cam_id, running, delay_dict, q, capture_test):
    try:
        capture_amount = 0
        while running.is_set():
        ...
        print(f"some error occured in trigger_master cam_id: {cam_id}, {e}")
        # print(f"camera {cam_id} captured image in {time.time()}")
...
def run_devices_test(cam_array, nums_cams, args):
    ...
    p.start()
    cam_array.StartGrabbing(py.GrabStrategy_LatestImageOnly)
    def trigger_master_test(args, cam, cam_id, running, delay_dict, q, capture_test):
        try:
            capture_amount = 0
            while running.is_set():
                print(f"master is running")
        ...
        try:
            for cam_id, cam in enumerate(cam_array):
                ...
                if cam_id != args.master:
                    ...
                    executor.submit(trigger_master_test, args, cam, cam_id, running, delay_dict, q, capture_test)
        except Exception as e:
...
def load_devices(args):
    ...
    if len(devs) > 0:
        ...
        for idx, cam in enumerate(cam_array):
            ...
            #py.FeaturePersistence_Load(FEATURE_DIR, cam.GetNodeMap()) # This throws timeout error on cameras
            cam.ExposureTime.SetValue(DEFAULT_EXPOSURE)
            cam.PixelFormat.SetValue('Mono8')
            cam.Width.SetValue(864)
            cam.Height.SetValue(864)
